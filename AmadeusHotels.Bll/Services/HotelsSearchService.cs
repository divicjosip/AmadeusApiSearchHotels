...
namespace AmadeusHotels.Bll.Services
{
    public class HotelsSearchService : IHotelsSearchService
    {
        ...
        private async Task SaveFetchedItems(HotelsSearchAmaduesFetchModel amaduesFetchModel, int searchRequestId, CancellationToken cancellationToken)
        {
            int fetchedItemsCount = amaduesFetchModel.Items.Count;
            if (amaduesFetchModel.Items != null && fetchedItemsCount > 0)
            {
                foreach (var fethModelItem in amaduesFetchModel.Items)
                {
                    var searchRequestHotel = _mapper.Map<SearchRequestHotel>(fethModelItem);
                    var hotel = _mapper.Map<Hotel>(fethModelItem.Hotel);
